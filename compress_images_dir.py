...
def compress_img(image_name, output_dir, new_size_ratio=0.9, quality=90, width=None, height=None, to_jpg=True):
    ...
    new_filename_res = output_dir + '/' + new_filename.split('\\')[-1]

    try:
        # save the image with the corresponding quality and optimize set to True
        img.save(new_filename_res, quality=quality, optimize=True)
    except OSError:
    ...
    print("[+] New file saved:", new_filename_res)
    # get the new image size in bytes

    new_image_size = os.path.getsize(new_filename_res)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-q", "--quality", type=int,
                        help="Quality ranging from a minimum of 0 (worst) to a maximum of 95 (best)."
                             " Default is 90", default=90)
    parser.add_argument("-r", "--resize-ratio", type=float,
                        help="Resizing ratio from 0 to 1, setting to 0.5 will multiply"
                             " width & height of the image by 0.5. Default is 1.0", default=1.0)
    ...
    if args.dir_input and args.dir_output:
        ...
        for subdir, dirs, files in os.walk(rootdir):
            for file in files:
                ...
                print("=" * 50)
                # compress the image
                compress_img(image, args.dir_output, args.resize_ratio, args.quality, args.width, args.height, args.to_jpg)
